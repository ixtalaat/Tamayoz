...
public class ServiceCatalogService(ApplicationDbContext db) : IServiceCatalogService
{
    ...
    public async Task<IReadOnlyList<Service>> GetAllAsync() => await db.Services.OrderByDescending(s => s.CreatedAt).ToListAsync();
    public Task<Service?> GetActiveByIdAsync(int id) => db.Services.FirstOrDefaultAsync(s => s.Id == id && s.IsActive);
    public Task<Service?> GetByIdAsync(int id) => db.Services.FindAsync(id).AsTask();
    public async Task CreateAsync(Service service) { service.CreatedAt = DateTime.UtcNow; db.Services.Add(service); await db.SaveChangesAsync(); }
    ...
    public async Task RemoveOrDeactivateAsync(int id)
    {
        ...
        if (service.Requests.Count == 0) db.Services.Remove(service);
        else { service.IsActive = false; service.UpdatedAt = DateTime.UtcNow; }
