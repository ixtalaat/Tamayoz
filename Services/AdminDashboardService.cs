...
{
    public async Task<AdminDashboardViewModel> GetAsync() => new()
    {
        TotalServices = await db.Services.CountAsync(), ActiveServices = await db.Services.CountAsync(s => s.IsActive),
        ...
        RecentRequests = await db.ServiceRequests.Include(r => r.Service).OrderByDescending(r => r.CreatedAt).Take(5).ToListAsync(),
        RecentMessages = await db.ContactMessages.OrderBy(m => m.IsRead).ThenByDescending(m => m.CreatedAt).Take(5).ToListAsync()
    };
}
